...
function generateTableOfContents() {
  ...
  sectionGroupings.forEach((section, sectionIndex) => {
    ...
    section.forEach((articleSlug, articleIndex) => {
      const articlePath = path.join(process.cwd(), 'src', 'copy', `${articleSlug}.md`);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const htmlPath = path.join(process.cwd(), 'temp-ebook.html');
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="Encountering-Messiah-Ebook.pdf"');
