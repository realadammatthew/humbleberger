...
function generateTableOfContents() {
  ...
  sectionGroupings.forEach((section, sectionIndex) => {
    ...
    section.forEach((articleSlug, articleIndex) => {
      const articlePath = path.join(__dirname, 'src', 'copy', 'he', `${articleSlug}.md`);
...
<body>
  ...
  encounteringMessiahArticles.forEach((articleSlug) => {
    ...
    if (fs.existsSync(articlePath)) {
      ...
      console.warn(`Warning: Article ${articleSlug}.md not found in Hebrew directory`);
    }
  });
...
async function generatePDF() {
  try {
    ...
    const htmlPath = path.join(__dirname, 'temp-ebook-he.html');
    ...
    const pdfPath = path.join(__dirname, 'public', 'encountering-messiah-ebook-he.pdf');
    fs.writeFileSync(pdfPath, pdf);
    
    console.log(`✅ Hebrew ebook generated successfully: ${pdfPath}`);
